...
const Header = () => {
	...
	return (
		<>
			<header
				className='fixed flex w-full p-4 transition-background ease-in-out duration-300 z-50 text-white'
				style={{
					color:
						routeOptions[router.asPath] && !scroll ? routeOptions[router.asPath].color : "white",
				...
				<animated.ul
					...
					{linksTransitions((styles, route) => (
						<animated.li style={styles}>
							<Link
								href={route.href}
								className='outline-white outline-1 hover:outline'
					...
					<animated.li
						...
						className='text-right outline-white outline-1 hover:outline'>
						<Link
							href={router.asPath}
				...
				<Link
					...
					locale={t("changeLangRoute")}
					scroll={false}
					className='mr-8 outline-white outline-1 hover:outline'>
					{t("changeLang")}
				</Link>
