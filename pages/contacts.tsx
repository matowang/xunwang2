  ...
  return (
    <div className="relative grid w-full overflow-hidden md:grid-cols-[34%_1fr]">
      <div className="fixed h-screen w-full md:relative ">
        ...
        <div className="absolute top-0 left-0 h-full w-full bg-gradient-to-r from-black to-[rgba(0,0,0,0.5)] md:hidden" />
      </div>
      <div className="relative w-full max-w-2xl justify-self-center px-10 py-40 sm:px-20">
...
const FormStates = () => {
  ...
  return stateTransition((styles, state) => {
    if (state === FormState.FILLING)
      return (
        <animated.div style={styles} className="absolute w-full">
    ...
    if (state === FormState.SENDING)
      return (
        ...
        >
          <p className="mb-10 mt-32 text-xl">{t("sending")}</p>
          <SquareSpinner />
        </animated.div>
      );
    if (state === FormState.SENT)
      return (
        <animated.div style={styles} className="absolute w-full">
          <p className="mb-10 mt-20 text-xl">
            <CheckCircleOutlineOutlinedIcon />{" "}
            {t("your form has been sent successfully")}
          </p>
          <Link href="/">
            <SlideButton className="self-start">
              {t("back to home page")}
              <LogoutOutlinedIcon />
            </SlideButton>
          </Link>
...
const ContactForm = ({
  ...
  useEffect(() => {
    if (!firstSubmit) validateForm({ email, name, message });
  }, [firstSubmit, email, name, message]);

  return (
    <form
      onSubmit={handleSubmit}
      className={`flex flex-col gap-10${loading ? " opacity-50" : ""}`}
    >
      ...
      >
        <SlideButton disabled={loading} type="submit">
          {t("send")}
        </SlideButton>
      </animated.div>
    </form>
