...
const FormStates = () => {
	...
	return stateTransition((styles, state) => {
		...
		if (state === FormState.SENDING)
			return (
				<animated.div style={styles} className='w-full absolute flex flex-col items-center'>
		...
		if (state === FormState.SENT)
			return (
				<animated.div style={styles} className='w-full absolute'>
					<p className='text-xl mb-10 mt-20'>
						<CheckCircleOutlineOutlinedIcon /> {t("your form has been sent successfully")}
					</p>
					<Link href='/'>
...
const ContactForm = ({
	...
	const validateForm = async (input: any): Promise<boolean> => {
		...
		try {
			...
			error.inner.forEach(
				(err: { path: "email" | "name" | "message" }) => (errors[err.path] = true)
			);
			setValidationErrors(errors);
			return false;
		}
